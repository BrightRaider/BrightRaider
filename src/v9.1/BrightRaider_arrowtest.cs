...
class BrightRaider : Form
{
    ...
    const int VK_INSERT = 0x2D; const int VK_DELETE = 0x2E;
    const int VK_LEFT = 0x25;   const int VK_RIGHT = 0x27;
    const int VK_UP = 0x26;     const int VK_DOWN = 0x28;
    ...
    // Overlay-Toggle: Entf (VK_DELETE, extended) — works in-game
    ...
    IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        ...
        if (nCode >= 0 && (wParam == (IntPtr)WM_KEYDOWN || wParam == (IntPtr)WM_SYSKEYDOWN))
        {
            ...
            bool isOcrToggle = false;

            if ((int)kb.vkCode == VK_ADD)
            {
                isCrosshair = true;
            }
            // Arrow keys + Delete: extended flag = real keys (not numpad)
            else if ((kb.flags & LLKHF_EXTENDED) != 0)
            {
                switch ((int)kb.vkCode)
                {
                    case VK_LEFT:   profile = 1; break;  // ← = Profil 1
                    case VK_DOWN:   profile = 2; break;  // ↓ = Profil 2
                    case VK_RIGHT:  profile = 3; break;  // → = Profil 3
                    case VK_UP:     isMute = true; break; // ↑ = Mute
                    case VK_INSERT: isCrosshair = true; break; // Ins = Fadenkreuz
                    case VK_DELETE: isOcrToggle = true; break; // Entf = Timer-Overlay
                }
            }
            ...
            if (isOcrToggle && isProLicensed && mapScannerEnabled)
            {
